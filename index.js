...
function sendError(action, store, next, result, status) {
  if (action.onError) {
    goTo(action.onError, store, next, result, { httpCode: status });
  }
...
const fetchMiddleware = store => next => (action) => {
  ...
  .then((response) => {
    if (response.ok) {
      ...
      if (action.autoDispatchPrefix) {
        next({ type: `${action.autoDispatchPrefix}_SUCCESS`, payload: response.body });
      }
    } else {
